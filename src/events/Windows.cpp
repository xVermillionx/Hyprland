                          ...
                          if(POSXSTR.contains('a')){      // absolute
                            ...
                            posX = g_pInputManager->getMouseCoordsInternal().x - PMONITOR->vecPosition.x +
                                (!POSXSTR.contains('%') ?
                                 std::stoi(POSXSTR) :
                                 (!POSXSTR.contains('c') ?
                                 std::stof(POSXSTR.substr(0, POSXSTR.length() - 1)) * 0.01 * PWINDOW->m_vRealSize.goal().x :
                                 std::stof(POSXSTR.substr(0, POSXSTR.length() - 2)) * 0.01 * PWINDOW->m_vRealSize.goal().x));
                                 if(POSXSTR.contains('c')) {
                                   if(posX+PWINDOW->m_vRealSize.goal().x > PMONITOR->vecSize.x){
                                     posX = PMONITOR->vecSize.x-PWINDOW->m_vRealSize.goal().x;
                                   }
                          ...
                          if(POSYSTR.contains('a')){      // absolute
                            ...
                            posY = g_pInputManager->getMouseCoordsInternal().y - PMONITOR->vecPosition.y +
                                (!POSYSTR.contains('%') ?
                                 std::stoi(POSYSTR) :
                                 (!POSYSTR.contains('c') ?
                                 std::stof(POSYSTR.substr(0, POSYSTR.length() - 1)) * 0.01 * PWINDOW->m_vRealSize.goal().y :
                                 std::stof(POSYSTR.substr(0, POSYSTR.length() - 2)) * 0.01 * PWINDOW->m_vRealSize.goal().y));
                                 if(POSYSTR.contains('c')) {
                                   if(posY+PMONITOR->vecPosition.y > PMONITOR->vecSize.y-PWINDOW->m_vRealSize.goal().y){
                                     posY = PMONITOR->vecSize.y-PWINDOW->m_vRealSize.goal().y;
                                   }
